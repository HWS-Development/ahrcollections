    ...
    <section id="home" className="relative h-screen min-h-[600px] md:min-h-[760px] w-full overflow-hidden bg-ink">
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center px-6 pb-20 sm:pb-24 md:pb-28 lg:pb-32 text-center">
        ...
        <motion.div
          ...
          className="mb-3 sm:mb-6 md:mb-8 inline-flex items-center justify-center"
        >
          <img
            src="/logos/royal-plaza-logo.svg"
            alt="Royal Plaza Hotel"
            className="h-20 sm:h-28 md:h-36 lg:h-44 w-auto drop-shadow-[0_10px_40px_rgba(0,0,0,0.75)]"
            draggable={false}
          />
        </motion.div>
        ...
        <div className="mt-4 sm:mt-6 md:mt-7 max-w-2xl text-ivory font-display text-[0.82rem] sm:text-[0.92rem] md:text-[0.98rem] leading-[1.5] sm:leading-[1.7] md:leading-[1.75] tracking-wide space-y-0.5 sm:space-y-1 [text-shadow:0_2px_18px_rgba(0,0,0,0.6)]">
          {t(HERO.body).map((line, i) => (
            <motion.p
              ...
              className={i >= 4 ? (i >= 6 ? 'hidden lg:block' : 'hidden sm:block') : ''}
            >
              {line}
            </motion.p>
          ))}
        ...
        <motion.div
          ...
          className="mt-5 sm:mt-7 md:mt-8 flex flex-wrap items-center justify-center gap-3 sm:gap-4"
        >
          <a
