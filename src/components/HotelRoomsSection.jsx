/**
 * HotelRoomsSection — generic cinematic rooms carousel.
 * Same visual DNA as Plaza's RoomsSection but fully data-injectable.
 *
 * Props:
 *   rooms      – array of room objects (label, tag, headline, description, amenities, photos)
 *   eyebrow    – bilingual { fr, en }
 *   title      – bilingual { fr, en }
 *   sub        – bilingual { fr, en }  (optional)
 *   bg         – Tailwind bg class (default 'bg-mist')
 *   bookingUrl – hotel-specific reservation link
 */
...
export default function HotelRoomsSection({
  rooms,
  eyebrow = { fr: 'Nos Espaces', en: 'Our Spaces' },
  title   = { fr: 'Des chambres pensées pour vous.', en: 'Rooms designed for you.' },
  sub,
  bg       = 'bg-mist',
  ...
  const AMENITIES_LABEL = { fr: 'Équipements', en: 'Amenities' };
  const PREV_LABEL      = { fr: 'Précédent',   en: 'Previous'  };
  const NEXT_LABEL      = { fr: 'Suivant',     en: 'Next'      };
  const BOOK_LABEL      = { fr: 'Réserver',    en: 'Book Now'  };

  /* Reset photo index when room tab changes */
  useEffect(() => { setPhotoIdx(0); setDir(1); }, [active]);
  ...
  const goNext = () => { setDir(1);  setPhotoIdx((i) => (i + 1) % photos.length); };
  const goPrev = () => { setDir(-1); setPhotoIdx((i) => (i - 1 + photos.length) % photos.length); };

  return (
    <section id="rooms" className={`relative section-pad ${bg} overflow-hidden`}>
      {/* Decorative blurs */}
      <div className="absolute -top-40 right-0 w-[500px] h-[500px] bg-champagne/10 blur-3xl pointer-events-none" />
      <div className="absolute -bottom-40 left-0 w-[500px] h-[500px] bg-bordeaux/10 blur-3xl pointer-events-none" />

      <div className="relative max-w-[1500px] mx-auto px-6 lg:px-12">

        {/* Heading */}
        <div className="text-center max-w-3xl mx-auto">
          <span className="ornament eyebrow">{t(eyebrow)}</span>
          <motion.h2
            initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }}
            className="mt-6 font-heading uppercase text-bordeaux text-[clamp(2.2rem,4.4vw,4rem)] leading-[1.05]"
          >
            {t(title)}
          </motion.h2>
          {sub && (
            <motion.p
              ...
              className="mt-6 font-display text-ink-soft text-[1rem] leading-[1.85]"
            >
              {t(sub)}
            </motion.p>
          )}
          ...
          <div className="lg:col-span-7 relative">
            <div className="relative cinema overflow-hidden shadow-deep h-[460px] md:h-[600px] bg-bordeaux-deep">
              ...
              <div className="absolute bottom-20 left-6 right-6 flex items-end justify-between text-ivory">
                ...
                <span className="hidden md:block font-heading text-champagne/70 text-[0.72rem] tracking-[0.4em] uppercase">
                  0{active + 1} / 0{rooms.length}
                </span>
              </div>

              {/* Arrows */}
              <button onClick={goPrev} aria-label={t(PREV_LABEL)}
              ...
              <button onClick={goNext} aria-label={t(NEXT_LABEL)}
              ...
              <div className="absolute bottom-6 left-6 right-6 flex items-center gap-4 text-ivory">
                ...
                <span className="font-heading text-[0.7rem] tracking-[0.4em] text-ivory/60">
            ...
            <div
              ...
              style={{ gridTemplateColumns: `repeat(${Math.min(photos.length, 7)}, minmax(0,1fr))` }}
            >
              {photos.slice(0, 7).map((src, i) => (
                <button
                  key={src + i}
                  onClick={() => { setDir(i > photoIdx ? 1 : -1); setPhotoIdx(i); }}
                  aria-label={`Photo ${i + 1}`}
                  className={`relative cinema aspect-[4/3] overflow-hidden shadow-soft transition-all duration-500 ${
                    photoIdx === i
                      ? 'ring-2 ring-champagne ring-offset-2 ring-offset-mist'
          ...
          <div className="lg:col-span-5 relative">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="mt-8">
                  <p className="eyebrow">{t(AMENITIES_LABEL)}</p>
                ...
                <div className="mt-auto pt-10 flex items-center justify-between">
                  <a href={bookingUrl} target="_blank" rel="noreferrer" className="btn-royal">
                    {t(BOOK_LABEL)} <span aria-hidden>→</span>
                  </a>
