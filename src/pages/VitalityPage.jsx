...
import {
  ...
  VITALITY_PRO_SERVICES, VITALITY_ROOM_AMENITIES, VITALITY_BOOKING_SECTION,
} from '../data/hotels.js';
...
export default function VitalityPage() {
  ...
  return (
    <>
      ...
      {/* ══════════════════════════════════════════
          ROOM AMENITIES — Standard / Premium tiers + included list
      ══════════════════════════════════════════ */}
      <section className="relative section-pad bg-ivory overflow-hidden">
        <div className="absolute inset-0 bg-champagne-radial pointer-events-none" />
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="text-center max-w-3xl mx-auto">
            <span className="ornament eyebrow">{t(VITALITY_ROOM_AMENITIES.eyebrow)}</span>
            <motion.h2
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }}
              className="mt-6 font-heading uppercase text-bordeaux text-[clamp(1.7rem,2.8vw,2.6rem)] leading-tight"
            >
              {t(VITALITY_ROOM_AMENITIES.title)}
            </motion.h2>
            <span className="block hairline mx-auto mt-6" />
            <p className="mt-6 font-display text-ink-soft text-[1rem] leading-[1.85]">
              {t(VITALITY_ROOM_AMENITIES.intro)}
            </p>
          </div>
          <div className="mt-14 grid md:grid-cols-2 gap-5 lg:gap-7">
            {VITALITY_ROOM_AMENITIES.tiers.map((tier, i) => (
              <motion.div
                key={i}
          ...
          <div className="mt-12 text-center">
            <span className="eyebrow !tracking-[0.32em]">{t(VITALITY_ROOM_AMENITIES.included.label)}</span>
            <div className="mt-5 flex flex-wrap justify-center gap-3">
              {VITALITY_ROOM_AMENITIES.included.items.map((it, i) => (
      ...
      {/* ══════════════════════════════════════════
          PRO SERVICES — Business Center, Coworking, Catering
      ══════════════════════════════════════════ */}
      <section className="relative section-pad bg-mist overflow-hidden">
        ...
        <div className="relative max-w-[1500px] mx-auto px-6 lg:px-12">
          <div className="text-center max-w-3xl mx-auto">
            <span className="ornament eyebrow">{t(VITALITY_PRO_SERVICES.eyebrow)}</span>
            <motion.h2
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }}
              className="mt-6 font-heading uppercase text-bordeaux text-[clamp(1.7rem,2.8vw,2.6rem)] leading-tight"
            >
              {t(VITALITY_PRO_SERVICES.title)}
            </motion.h2>
            <span className="block hairline mx-auto mt-6" />
          </div>
          <div className="mt-14 grid md:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
            {VITALITY_PRO_SERVICES.items.map((it, i) => (
              <motion.div
                key={i}
      ...
      {/* ══════════════════════════════════════════
          BOOKING SECTION — Réservation en Ligne
      ══════════════════════════════════════════ */}
